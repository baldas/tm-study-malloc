...
stm_init(void)
{
  ...
  memset((void *)_tinystm.addr, 0, LOCK_ARRAY_SIZE * sizeof(stm_word_t));
#endif
  
#ifdef TM_STATISTICS
  memset((void *)&_tinystm.stats, 0, sizeof(stats_data_t));
#endif
...
#ifdef TM_STATISTICS
  printf("#commits      : %lu \n", _tinystm.stats.nb_commits);
#ifdef TRACK_FALSE_CONFLICT
  float ratio = _tinystm.stats.nb_true_conflicts ? (float)_tinystm.stats.nb_false_conflicts/(float)(_tinystm.stats.nb_true_conflicts+_tinystm.stats.nb_false_conflicts)*100 : 0.0;
  printf("#aborts       : %lu true: %lu  false: %lu ( %.2f )\n", _tinystm.stats.nb_aborts,
          _tinystm.stats.nb_true_conflicts, _tinystm.stats.nb_false_conflicts, ratio);
  if (_tinystm.stats.nb_aborts != _tinystm.stats.nb_true_conflicts+_tinystm.stats.nb_false_conflicts)
    printf("aborts and true+false conflicts differ!\n");
#else
  printf("#aborts       : %lu \n", _tinystm.stats.nb_aborts);
#endif
  printf("  #lock-r     : %lu \n", _tinystm.stats.nb_aborts_locked_read);
  printf("  #lock-w     : %lu \n", _tinystm.stats.nb_aborts_locked_write);
  printf("  #val-r      : %lu \n", _tinystm.stats.nb_aborts_validate_read);
  printf("  #val-w      : %lu \n", _tinystm.stats.nb_aborts_validate_write);
  printf("  #val-c      : %lu \n", _tinystm.stats.nb_aborts_validate_commit);
  printf("  #inv-mem    : %lu \n", _tinystm.stats.nb_aborts_invalid_memory);
  printf("#aborts>=1    : %lu \n", _tinystm.stats.nb_aborts_1);
  printf("#aborts>=2    : %lu \n", _tinystm.stats.nb_aborts_2);
#endif

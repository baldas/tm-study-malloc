...
#ifdef NEW_HASH_FUNCTION
# define LOCK_IDX(a)                    ( (((stm_word_t)a & 0xFFFFFFFF) ^ ((stm_word_t)a >> 32 )) & LOCK_MASK)
#else
...
#ifdef TRACK_FALSE_CONFLICT
#define GET_LOCKED_ADDR(a)		(_tinystm.addr + LOCK_IDX(a))
#endif
...
#ifdef TRACK_FALSE_CONFLICT
  unsigned long nb_true_conflicts;
...
#ifdef TRACK_FALSE_CONFLICT
  volatile stm_word_t *addr;            /* Pointer to address */
#endif
} r_entry_t;
...
#ifdef TRACK_FALSE_CONFLICT
  unsigned int nb_false_conflicts;      /* Number of false conflicts in the ORT */
  unsigned int nb_ok;                   /* "true" conflicts */  
#endif
} stm_tx_t;
...
#endif /* CM == CM_MODULAR */
#ifdef TM_STATISTICS
  stats_data_t stats;            /* global stats */
#endif /* TM_STATISTICS */
#ifdef TRACK_FALSE_CONFLICT
  volatile stm_word_t addr[LOCK_ARRAY_SIZE] ALIGNED; /* stores the last address that caused the update 
							of the version word in the ORT */ 
#endif
...
int_stm_exit_thread(stm_tx_t *tx)
{
  ...
  */
  unsigned long nb_commits = 0; 
  unsigned long nb_aborts = 0;
  unsigned long nb_aborts_1 = 0;
  unsigned long nb_aborts_2 = 0;
  unsigned long nb_aborts_locked_read = 0;
  unsigned long nb_aborts_locked_write = 0;
  unsigned long nb_aborts_validate_read = 0;
  unsigned long nb_aborts_validate_write = 0;
  unsigned long nb_aborts_validate_commit = 0;
  unsigned long nb_aborts_invalid_memory = 0;

  stm_get_stats("nb_commits", &nb_commits);
  stm_get_stats("nb_aborts", &nb_aborts);
  stm_get_stats("nb_aborts_1", &nb_aborts_1);
  stm_get_stats("nb_aborts_2", &nb_aborts_2);
  stm_get_stats("nb_aborts_locked_read", &nb_aborts_locked_read);
  stm_get_stats("nb_aborts_locked_write", &nb_aborts_locked_write);
  stm_get_stats("nb_aborts_validate_read", &nb_aborts_validate_read);
  stm_get_stats("nb_aborts_validate_write", &nb_aborts_validate_write);
  stm_get_stats("nb_aborts_validate_commit", &nb_aborts_validate_commit);
  stm_get_stats("nb_aborts_invalid_memory", &nb_aborts_invalid_memory);

  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_commits, nb_commits);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts, nb_aborts);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_1, nb_aborts_1);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_2, nb_aborts_2);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_locked_read, nb_aborts_locked_read);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_locked_write, nb_aborts_locked_write);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_validate_read, nb_aborts_validate_read);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_validate_write, nb_aborts_validate_write);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_validate_commit, nb_aborts_validate_commit);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_aborts_invalid_memory, nb_aborts_invalid_memory);

#ifdef TRACK_FALSE_CONFLICT
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_true_conflicts, tx->nb_ok);
  ATOMIC_FETCH_ADD_FULL(&_tinystm.stats.nb_false_conflicts, tx->nb_false_conflicts);
#endif

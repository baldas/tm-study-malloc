...
static INLINE void
track_false_conflict(stm_tx_t *tx, volatile stm_word_t *addr)
{
  volatile stm_word_t *laddr = GET_LOCKED_ADDR(addr);
  if ((stm_word_t *) *laddr != addr)
...
#ifdef TRACK_FALSE_CONFLICT
        track_false_conflict(tx, r->addr);
#endif
...
#ifdef TRACK_FALSE_CONFLICT
    track_false_conflict(tx, addr);
#endif

    stm_rollback(tx, STM_ABORT_WR_CONFLICT);
    return 0;
  } else {
    /* Not locked */
...
#endif /* CM == CM_MODULAR */
#ifdef TRACK_FALSE_CONFLICT
	if (tx->attr.read_only) track_false_conflict(tx, addr);
#endif	
       /* If the transaction aborts because of stm_wbetl_extend, the false conflict 
	* counting will be taken care of there */

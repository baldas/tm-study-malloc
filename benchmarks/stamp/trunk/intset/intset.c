 ...
 */
# define TM_LOAD(addr)                      TM_SHARED_READ(addr)
# define TM_STORE(addr, value)              TM_SHARED_WRITE(addr, value)
# define TM_COMMIT                          TM_END()

# define TM_INIT                            TM_STARTUP(0)
# define TM_EXIT                            TM_SHUTDOWN()
# define TM_INIT_THREAD                     TM_THREAD_ENTER()
# define TM_EXIT_THREAD                     TM_THREAD_EXIT()

#if defined(IS_WLPDSTM)
#  define TM_ARG_NULL                         (void **)0,
#else
#  define TM_ARG_NULL			    /* nothing */
#endif
...
TM_SAFE
static node_t *new_node(TM_ARGDECL val_t val, node_t *next, int transactional)
{
  node_t *node;

  if (!transactional) {
    node = (node_t *)SEQ_MALLOC(sizeof(node_t));
  } else {
...
static intset_t *set_new()
{
  ...
  if ((set = (intset_t *)SEQ_MALLOC(sizeof(intset_t))) == NULL) {
    perror("malloc");
    exit(1);
  }
  max = new_node(TM_ARG_NULL VAL_MAX, NULL, 0);
  min = new_node(TM_ARG_NULL VAL_MIN, max, 0);
  set->head = min;
...
static int set_contains(TM_ARGDECL intset_t *set, val_t val, thread_data_t *td)
{
  int result;
  node_t *prev, *next;
  val_t v;
...
static int set_add(TM_ARGDECL intset_t *set, val_t val, thread_data_t *td)
{
  int result;
  node_t *prev, *next;
  val_t v;
...
# endif
  ...
  if (td == NULL) {
    ...
    result = (next->val != val);
    if (result) {
      prev->next = new_node(TM_ARG_NULL val, next, 0);
    }
  } else if (td->unit_tx == 0) {
    //TM_START(RW);
    TM_BEGIN();
    ...
    result = (v != val);
    if (result) {
      TM_STORE(prev->next, new_node(TM_ARG val, next, 1));
    }
    TM_COMMIT;
  } 
//#ifndef TM_COMPILER
...
static int set_remove(TM_ARGDECL intset_t *set, val_t val, thread_data_t *td)
{
  int result;
  node_t *prev, *next;
  val_t v;
...
# include "rbtree-104.h"

# include "rbtree-104.c"
...
# endif
  ...
  if (!td) {
    ...
    //TM_START(RO);
    TM_BEGIN_RO();
    result = TMrbtree_contains(TM_ARG (rbtree_t *)set, (void *)val);
    TM_COMMIT;
  }
...
# endif
  ...
  if (!td) {
    ...
    //TM_START(RW);
    TM_BEGIN();
    result = TMrbtree_insert(TM_ARG (rbtree_t *)set, (void *)val, (void *)val);
    TM_COMMIT;
  }
...
# endif
  ...
  if (!td) {
    ...
    //TM_START(RW);
    TM_BEGIN();
    result = TMrbtree_delete(TM_ARG (rbtree_t *)set, (void *)val);
    TM_COMMIT;
  }
...
TM_SAFE
static node_t *new_node(TM_ARGDECL val_t val, level_t level, int transactional)
{
  node_t *node;

  if (!transactional) {
    node = (node_t *)SEQ_MALLOC(sizeof(node_t) + level * sizeof(node_t *));
  } else {
...
static intset_t *set_new(level_t max_level, int prob)
{
  ...
  set->tail = new_node(TM_ARG_NULL VAL_MAX, max_level, 0);
  set->head = new_node(TM_ARG_NULL VAL_MIN, max_level, 0);
...
# endif
  ...
  if (!td) {
    ...
    if (node->val == val) {
      ...
      node = new_node(TM_ARG_NULL val, l, 0);
      for (i = 0; i <= l; i++) {
...
TM_SAFE
static bucket_t *new_entry(TM_ARGDECL val_t val, bucket_t *next, int transactional)
{
  bucket_t *b;

  if (!transactional) {
    b = (bucket_t *)SEQ_MALLOC(sizeof(bucket_t));
  } else {
...
# endif
  ...
  if (!td) {
    ...
    if (result) {
      set->buckets[i] = new_entry(TM_ARG_NULL val, first, 0);
    }
  } else {
    //TM_START(RW);
    TM_BEGIN();
    ...
    if (result) {
      TM_STORE(set->buckets[i], new_entry(TM_ARG val, first, 1));
    }
    TM_COMMIT;
  }
...
#ifdef THREAD_AFFINITY
    if (stick_this_thread_to_core(thread_to_core_map[d->tid])) {
    ...
    if (op < d->update) {
      if (d->alternate) {
        ...
        if (last < 0) {
          ...
          /* Remove last value */
          if (set_remove(TM_ARG d->set, last, d))
            d->diff--;
          d->nb_remove++;
          last = -1;
        }
        ...
        if ((op & 0x01) == 0) {
          ...
          /* Remove random value */
          if (set_remove(TM_ARG d->set, val, d))
            d->diff--;
          d->nb_remove++;
        }
      }
      ...
      val = rand_range(d->range, d->seed) + 1;
      if (set_contains(TM_ARG d->set, val, d))
        d->nb_found++;
      d->nb_contains++;
    }
  }
...
int main(int argc, char **argv)
{
  ...
  if ((data = (thread_data_t *)SEQ_MALLOC(nb_threads * sizeof(thread_data_t))) == NULL) {
    perror("malloc");
    exit(1);
  }
  if ((threads = (pthread_t *)SEQ_MALLOC(nb_threads * sizeof(pthread_t))) == NULL) {
    perror("malloc");
    exit(1);
  }
...
//#ifndef TM_COMPILER
  ...
  while (i < initial) {
    ...
    if (set_add(TM_ARG_NULL set, val, 0))
      i++;
  }
  size = set_size(set);
  printf("Set size     : %d\n", size);
  ...
  unsigned int native[2];
  
  native[0] = PAPI_L1_DCA;
  native[1] = PAPI_L1_DCM; 
  //native[2] = PAPI_TOT_INS;
  //native[3] = PAPI_TOT_CYC;

  if ( ( retval = PAPI_add_event( EventSet, native[0] ) ) != PAPI_OK ) {
    perror("PAPI_add_event");
    exit(-1);
  }
  
  if ( ( retval = PAPI_add_event( EventSet, native[1] ) ) != PAPI_OK ) {
    perror("PAPI_add_event");
    exit(-1);
  }
  /*
  if ( ( retval = PAPI_add_event( EventSet, native[2] ) ) != PAPI_OK ) {
    perror("PAPI_add_event");
    exit(-1);
  }
  
  if ( ( retval = PAPI_add_event( EventSet, native[3] ) ) != PAPI_OK ) {
    perror("PAPI_add_event");
    exit(-1);
  }
  */
...
#ifdef USE_PAPI
  fprintf(stdout, "#accesses/misses: %llu %llu ( %.2f%% )\n", values[0], values[1], 
		  ((double)values[1]/(double)values[0])*100.0);
  //fprintf(stdout, "TOTINST: %llu\n", values[2]);
  //fprintf(stdout, "UREFCYC: %llu\n", values[3]);
  //printf("#accesses/misses: %llu %llu (%.2f%%)\n", total_L1d_accesses, total_L1d_misses, 
//		  ((double)total_L1d_misses/(double)total_L1d_accesses)*100.0);
#endif
//#ifndef TM_COMPILER
#if 0
